...
当请求跨越 09:25 返回，保留真实接收时间，不倒填成 09:24:59。09:25–09:26:50 是终态复核窗口：`finalized`（全部代码已取得一次终态）不再终止补采，调度器按轮继续复核并在同一会话采用最新值，因为官方首个 `matched/final` 可能仍是旧值（2026-09-23 实测 76 只中 54 只更晚才落定）；复核轮同样逐批落盘，形成可审计的终值修订链。实时端点每批不在网络层重试，下一轮按调度重试；一般 REST 最多重试 3 次。限流和无效密钥不会立即重复轰炸接口。复盘在独立线程进行，09:10–09:26 禁止开始新的复盘任务。系统不宣称操作系统调度或网络具有硬实时保证。
...
趋势池按截止日匹配上一交易日。`session_trends` 保留本次会话使用的旧池，收盘后新生成的下一次池不会反向改写本次竞价来源；同一份副本原子写入 `data/trend-pool-session.json`，因此进程在同日重启后（即便收盘刷新已经覆盖 `trend-pool.json`）仍按原会话的昨日趋势来源重建关注池，不会静默退化成只有昨日涨停的子集。快照只在日期等于会话上一交易日时生效，日期不符按无池处理，不推断来源也不补造观察。纯自选模式不自动合入涨停或趋势池。趋势筛选以 `should_stop` 检查关闭和保护时段，中断后清除本次自动尝试标记以便稍后重试；同日已完成池不会被中断的partial结果覆盖。
...
| `data/trend-pool-session.json` | 本次会话使用的昨日趋势池副本；仅供同日重启保留原会话关注来源，日期不符或内容非法即忽略，不参与评分与档案 |
