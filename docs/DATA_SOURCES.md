...
涨跌分布及成交额统计只覆盖有效行情样本；涨停聚集则以全部有效当前成员与原报告经核验的完整收盘池相交，两者分母不同。严格连板未知不填0，另给`consecutive_known_count`；成分不完整和池缺失分别提示。每板JSON最多100同行/30涨停、AI60/60、页面20/20，截断不改变底层统计分母，也不能以未展示认定未涨停。全部无有效证据时状态为unavailable并停止模型调用。
...
市场快照没有名称时由官方代码表补充；无法确认名称、ST/退市标记或代码格式不合格的候选不参与自动筛选。每只候选只读取截至目标日的最近60自然日前复权日线；硬阈值与筛选评分见策略文档。`trend-pool.json` 保存候选数、评估数、有效历史数、未评估数、入选数及质量提示，不能把有限样本称为全市场搜索。`trend-pool-session.json` 只是本次会话所用昨日池的副本，用于同日重启保留原会话关注来源；它不是新证据，也不覆盖 `trend-pool.json`。
...
2026-09-21真实交易日留存的认证响应使用 `order_entry/live`、`no_cancel/live`、`matched/final` 三组阶段/状态，而官网当前字段表只把两个字段描述为字符串，示例使用 `final/ready`。系统按实测时段把这三组加入明确白名单，同时继续兼容已核实的旧命名；不会把任意新字符串或矛盾组合猜成就绪。原字段始终随批次保存，归一化只控制能否参与评分。
...
2026-09-23 又观察到 `matched/final` 的时间语义：09:25:02 首次返回的 `matched/final` 仍是集合竞价最后一刻的旧值（`auction_pct` 与 09:24:57 的 live 快照完全相同），而当日 09:36 再取同一批 76 只，**54 只的终值不同**（南华生物 9.744%→0.00%、华远控股 10.1563%→9.375%、大亚圣象 10.0145%→5.9507%，成交额普遍更大）。因此“阶段/状态为终态”不等于“数值已落定”，调度器把终态复核窗口延长到 09:26:50 并持续采用最新值；这是同交易日内的终值复核，不是历史日期查询（该接口没有历史参数）。事后如需人工核对，可用 `python tools/late_final_snapshot.py`（仅当日）生成 `data/research/late-final/YYYY-MM-DD-*.json/.md`：列出补录终值与冻结档案的逐只差异，明确标注不是原始接收证据。
...
`official_context.py`最多调用4个业务分项：风向标一次、龙虎榜三类各一次；全部显式传目标报告的date，并在每个分项前检查取消、模式与保护时段。收盘复盘（自动15:10与手动相同）就会调用这四项并写入新报告；手动「联网补充官方观察」用于重试或晚间刷新，已有ready不被新partial覆盖。每个provider.get仍可能常规重试，不能把4个分项写成总共4次HTTP请求。该模块仍不扫描额外年份、概念成员或每只股票日线，也不在竞价轮询内调用。
