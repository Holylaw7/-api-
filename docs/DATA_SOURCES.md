...
2026-09-23 又观察到 `matched/final` 的时间语义：09:25:02 首次返回的 `matched/final` 仍是集合竞价最后一刻的旧值（`auction_pct` 与 09:24:57 的 live 快照完全相同），而当日 09:36 再取同一批 76 只，**54 只的终值不同**（南华生物 9.744%→0.00%、华远控股 10.1563%→9.375%、大亚圣象 10.0145%→5.9507%，成交额普遍更大）。因此“阶段/状态为终态”不等于“数值已落定”，调度器把终态复核窗口延长到 09:26:50 并持续采用最新值；这是同交易日内的终值复核，不是历史日期查询（该接口没有历史参数）。
